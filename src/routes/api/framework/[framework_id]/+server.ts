...
// GitHub API calls
async function fetchGitHubData(owner: string, repo: string, errors: APIError[]): Promise<Partial<FrameworkStats['github']>> {
  ...
  try {
    ...
    let repoData = frameworkCache.get(cacheKey);
  ...
  try {
    ...
    let activityData = frameworkCache.get(cacheKey);
  ...
  if (result.pushed_at && result.open_issues !== undefined && result.stars) {
    const daysSinceLastPush = (Date.now() - new Date(result.pushed_at).getTime()) / (1000 * 60 * 60 * 24);
...
// NPM API calls
async function fetchNPMData(packageName: string, errors: APIError[]): Promise<Partial<NPMData>> {
  ...
  try {
    ...
    if (packageResponse.ok) {
      ...
      result.dependencies_count = Object.keys(packageData.dependencies || {}).length;
      result.dev_dependencies_count = Object.keys(packageData.devDependencies || {}).length;
      result.peer_dependencies_count = Object.keys(packageData.peerDependencies || {}).length;
...
// Bundle Phobia API
async function fetchBundleData(packageName: string, errors: APIError[]): Promise<Partial<BundleData>> {
  ...
  try {
    ...
    if (bundleResponse.ok) {
      ...
      result.dependencies = bundleData.dependencies?.map((dep: any) => ({
        name: dep.name,
...
async function fetchSecurityData(packageName: string, errors: APIError[]): Promise<Partial<SecurityData>> {
  ...
  try {
    ...
    if (securityResponse.ok) {
      ...
      result.vulnerabilities = vulnerabilities.reduce((acc: any, vuln: any) => {
        const severity = vuln.database_specific?.severity?.toLowerCase() || 'low';
        if (severity.includes('critical')) acc.critical++;
        else if (severity.includes('high')) acc.high++;
        else if (severity.includes('moderate') || severity.includes('medium')) acc.moderate++;
        else acc.low++;
        acc.total++;
        return acc;
...
function calculateEcosystemHealth(
  ...
  const npm_popularity_score = Math.min(100,
    (Math.log(npm_downloads + 1) * 5) +
    (Math.log(github_stars + 1) * 3)
  );

  // Maintenance score based on recent activity
  const days_since_push = github.pushed_at ?
    (Date.now() - new Date(github.pushed_at).getTime()) / (1000 * 60 * 60 * 24) : 365;
  ...
  const issue_resolution = github.open_issues ?
    Math.max(0, 100 - (github.open_issues / Math.max(github.stars || 1, 1) * 100)) : 50;
  const community_score = Math.min(100, (contributors_count * 5) + (issue_resolution / 2));
  ...
  const security_score = Math.max(0, 100 - (critical_vulns * 30) - (high_vulns * 15) - (vuln_count * 2));

  // Overall health (weighted average)
  const overall_health = Math.round(
    (github_activity_score * 0.25) +
    (npm_popularity_score * 0.20) +
...
export const GET: RequestHandler = async ({ params, request }) => {
  ...
  if (!rateLimiter.isAllowed(clientIP)) {
    return json(
      ...
      },
      {
        status: 429,
        headers: {
          'Retry-After': String(Math.ceil((rateLimiter.getResetTime(clientIP) - Date.now()) / 1000)),
  ...
  try {
    ...
    const [githubData, npmData, bundleData, securityData] = await Promise.allSettled([
      fetchGitHubData(frameworkMeta.github.owner, frameworkMeta.github.repo, errors),
      fetchNPMData(frameworkMeta.npm.packageName, errors),
      fetchBundleData(frameworkMeta.bundle.bundlephobiaName || frameworkMeta.npm.packageName, errors),
